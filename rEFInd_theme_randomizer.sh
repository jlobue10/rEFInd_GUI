#!/bin/bash
# Swaps themes/active_theme.conf on the EFI system partition that firmware
# actually launches rEFInd from for a random installed theme's theme.conf.
# Runs as root via rEFInd_theme_randomizer.service. Unlike the background
# randomizer, this picks from the ESP's own EFI/refind/themes tree rather
# than the user's data directory: the copied theme.conf references assets by
# themes/<name>/... paths, so only themes already installed on the ESP are
# valid choices -- and reading only root-owned ESP files keeps this boot-time
# root service away from user-writable input entirely. The ESP resolution
# mirrors install_config_from_GUI.sh (and the Windows randomizer) -- these
# scripts are deliberately standalone; keep them in sync.
...
THEMES_DIR="$ESP/EFI/refind/themes"
ACTIVE="$THEMES_DIR/active_theme.conf"
[ -d "$THEMES_DIR" ] || exit 0

# Candidates: every installed themes/<name>/theme.conf. With more than one
# available, avoid re-picking the one that is already active.
CANDIDATES=()
...
if [ "${#CANDIDATES[@]}" -gt 1 ] && [ -f "$ACTIVE" ]; then
	FRESH=()
	for conf in "${CANDIDATES[@]}"; do
		cmp -s "$conf" "$ACTIVE" || FRESH+=("$conf")
	done
	[ "${#FRESH[@]}" -gt 0 ] && PICK_FROM=("${FRESH[@]}")
fi
RAND_CONF="$(printf '%s\n' "${PICK_FROM[@]}" | shuf -n1)"

if [ -n "$RAND_CONF" ]; then
	esp_make_writable "$ESP"
	# Note this only changes which theme an already theme-enabled refind.conf
	# includes; a config created with Theme = None has no include line and is
	# unaffected.
	cp -f "$RAND_CONF" "$ACTIVE"
