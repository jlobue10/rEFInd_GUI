...
Other devices are unaffected — the driver is only installed when an Xbox Ally / Ally X (DMI board `RC73XA`/`RC73YA`), Steam Deck (DMI product `Galileo` for OLED, `Jupiter` for LCD) or ASUS Zenbook Pro 14 Duo (DMI board `UX8402VV`; ELAN9008 panel on Intel Serial IO I2C, supported since driver v1.3.0 — hardware validation in progress) is detected. Like the controller driver, it's fetched from `releases/latest` at install time, and a failed download only skips it. Installs that previously received `AllyTouchI2cDxe.efi` get it replaced by `TouchI2cDxe.efi` on the next rEFInd install.
