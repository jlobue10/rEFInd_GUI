...
try {
    ...
    # Staged, publish-last: optional assets first and refind.conf last.
    # Move-Item stays on the ESP, so each live file is replaced only after its
    # copy completed (an overwriting Move-Item is delete-then-move, not an
    # atomic swap, hence the config is published last).
