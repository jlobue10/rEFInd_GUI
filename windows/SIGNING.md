...
| `SIGNPATH_PROJECT_SLUG` | `rEFInd_GUI` |
...
2. **Stage A** — upload `deploy/` and submit it under `deploy-contents`;
   SignPath returns the folder with `rEFInd_GUI.exe` and the `.ps1` scripts
...
```powershell
Get-AuthenticodeSignature .\rEFInd_GUI.exe | Format-List Status, SignerCertificate
signtool verify /pa /v .\rEFInd_GUI-2.0.0-setup.exe   # if the Windows SDK is installed
```
