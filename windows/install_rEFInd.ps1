...
# %LOCALAPPDATA%\rEFInd_GUI\bootmgr-backup.txt.
...
function New-InstallerStagingDirectory {
    ...
    Get-ChildItem -LiteralPath $tempRoot -Directory `
            -Filter 'rEFInd_GUI-Staging-*' -ErrorAction SilentlyContinue |
        ForEach-Object {
            Remove-Item -Recurse -Force -LiteralPath $_.FullName -ErrorAction SilentlyContinue
        }

    $path = Join-Path $tempRoot ('rEFInd_GUI-Staging-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $src = Join-Path $env:LOCALAPPDATA 'rEFInd_GUI\GUI'
    ...
    $backupDir = Join-Path $env:LOCALAPPDATA 'rEFInd_GUI'
...
# rEFInd's binaries are not Microsoft-signed, so Secure Boot firmware may
# refuse to load them. The Linux install scripts handle the sbctl-managed case
# (CachyOS) by signing right after installing; that signing can only be done
# from Linux, so from here the best we can do is check and warn.
# Confirm-SecureBootUEFI needs the elevation this script already has; it
# throws on legacy-BIOS/unsupported systems, which counts as "not enabled".
$secureBootOn = $false
try { $secureBootOn = [bool](Confirm-SecureBootUEFI) } catch {}
if ($secureBootOn) {
    Write-Host ''
    Write-Host 'NOTE: Secure Boot is ENABLED on this system.' -ForegroundColor Yellow
    Write-Host 'If rEFInd does not appear at boot, the firmware is refusing its unsigned' -ForegroundColor Yellow
    Write-Host 'binaries. If your Linux install manages Secure Boot with sbctl (e.g.' -ForegroundColor Yellow
    Write-Host 'CachyOS), boot into Linux and sign the newly installed files with:' -ForegroundColor Yellow
    Write-Host '    sudo sbctl-batch-sign' -ForegroundColor Yellow
    Write-Host 'Otherwise enroll rEFInd via shim/MOK or your firmware, or disable Secure Boot.' -ForegroundColor Yellow
}
...
if ($backup) {
    ...
    Write-Host "  powershell -ExecutionPolicy Bypass -File `"$env:ProgramFiles\rEFInd_GUI\windows\uninstall_rEFInd.ps1`""
    Write-Host 'or uninstall "rEFInd GUI" from Windows Settings > Apps.'
}
