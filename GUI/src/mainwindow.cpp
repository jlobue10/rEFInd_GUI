...
static const char APP_VERSION[] = "3.4.4";
...
// First-run defaults: the platform's preferred OS leads (Windows on the
// Windows build, the running distro on Linux), then the others. Entries pack
// into slots 1, 2, ... with no gaps; the leading OS is also the default boot.
void MainWindow::applyAutoSelection()
{
    ...
    if (Platform::preferWindowsAsDefault())
        ordered << windowsPick << linuxPick << steamPick;
    else
        ordered << linuxPick << windowsPick << steamPick;
...
void MainWindow::on_Preview_pushButton_clicked()
{
    ...
    if (themeChoice == QLatin1String(kRandomThemeData)) {
        const QStringList themes = availableThemes();
...
// never runs when the app is force-terminated (Xbox overlay / task switcher,
...
void MainWindow::readSettings()
{
    ...
    // with the override later disarmed — always win. Deferred here because
    // panelPrefill is computed on the detection worker thread.
...
void MainWindow::on_About_pushButton_clicked()
{
    ...
    aboutBox.setText(tr("<p align='center'>rEFInd Customization GUI v%1<br><br>"
                        "Original GUI Creator: "
                        "<a href='https://github.com/jlobue10'>jlobue10</a><br><br>"
                        "Special Thanks to Deck Wizard for testing and QA"
                        "<br><br><a href='https://www.youtube.com/watch?v=ubWPIf2DbvE'>"
...
void MainWindow::onUpdateReply(bool ok, const QString &remoteRaw, const QString &errorString)
{
    ...
    if (!ok || remote.isNull()) {
        ...
        updateBox.setText(tr("<p align='center'>An update is available "
                             "<a href='https://github.com/jlobue10/rEFInd_GUI/releases'>here</a>"
                             "<br><br></p>"));
    } else {
...
void MainWindow::toggleBackgroundRandomizer(bool enable)
{
    if (!Platform::setBackgroundRandomizer(enable))
...
{
    toggleBackgroundRandomizer(true);
}
