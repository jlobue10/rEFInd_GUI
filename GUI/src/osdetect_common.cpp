...
bool OSDetector::isRunningSystemEsp(const Partition &p)
{
    ...
    // these paths never match a drive-letter mount point, so this is false
    // there (where a running-distro name is unavailable anyway).
    return p.mountPoint == QLatin1String("/boot")
